...
class ForloebTaskConsoleController extends ControllerBase {
  public const WS_TOKEN_NAME = 'os2forms-forloeb-ws-token';
  ...
  public function execute() {
    ...
    $token = $this->requestStack->getCurrentRequest()->query->get(self::WS_TOKEN_NAME, '');
    if ($token) {
    ...
    switch ($handler_type) {
      ...
      case 'function':
        if ($token) {
          $query_options[self::WS_TOKEN_NAME] = $token;
        }
